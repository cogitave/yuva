...
pub fn timer_demo() -> bool {
    ...
    // 2. Enable the LAPIC (which also masks every non-timer LVT — see
    //    `lapic_enable`, the fix for the microvm+KVM first-`sti` #DE) + arm the
    //    periodic timer (still masked by IF=0).
    lapic_enable();
    lapic_timer_arm();
