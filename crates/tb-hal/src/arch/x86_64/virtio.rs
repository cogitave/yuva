...
// guest<->host inference-transport seam. BYTE-SYMMETRIC with the aarch64 arm:
// same two-queue layout, same posted-rx-before-DRIVER_OK discipline, same
// VERSION_1-only negotiation (F_MULTIPORT/F_SIZE/F_EMERG_WRITE all rejected,
// so §5.3.2 pins the device to receiveq(0)+transmitq(1) on port 0), same
// POLL_CAP-bounded used-ring spins (`mode=POLL`; the #71 watch applies the
// instant this grows a completion IRQ). The ONLY x86 deltas are `dma_fence`
// (TSO) where aarch64 uses dmb/dsb, and the UC device-window `map_window()`
// (aarch64 needs none). The bytes moved are OPAQUE here: ALL framing/MAC
// discipline is the Kani-proven `tb_encode::inferwire`, called by the safe
// selftest layer (the M30 proposal §1: "the codec is the verified leaf; the
// transport is plumbing").
// ===========================================================================
...
/// (DeviceID==3). Maps the UC device window first (shared with M19/M20).
/// Returns `Some(slot)` if found and modern, `None` if absent, legacy-only,
/// or the window could not be mapped. Mirrors [`blk_probe`].
...
/// See the aarch64 twin for the full contract; this arm is byte-identical save
/// the `dma_fence` (TSO) barriers and the UC window mapping. Returns
/// `Some(resp.len())` on success, `None` fail-closed on ANY fault (the caller
/// sizes `resp` to the exact expected response byte count).
pub fn chan_send_recv(slot: u32, req: &[u8], resp: &mut [u8]) -> Option<usize> {
    ...
    while written < resp.len() {
        ...
        dma_fence(); // order the len/data loads after the used.idx load
