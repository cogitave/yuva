...
//! MV (the L1 sovereignty rung) adds a SECOND x86_64 entry in `boot`: a brand
//! ELF note (PT_NOTE name "YUVA", type 0x59550001 — both derived from
...
//! QEMU/Firecracker use the Xen note, tb-vmm uses the brand note, one runs per
//! boot.
...
// M30: the virtio-console CHANNEL surface the safe inference-transport
// selftest layer calls (the only new x86 unsafe lives in virtio.rs alongside
// the M19/M20 paths).
pub use virtio::{chan_probe, chan_saw_legacy, chan_send_recv};
