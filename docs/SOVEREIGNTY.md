# TABOS Sovereignty and Clean-Slate Decision
...
> This document answers that question **sourced and honestly**: it draws what is silicon-mandatory, what is a neutral open standard, what is rejected Linux heritage, and what belongs to TABOS.
...
**Yes — the kernel is 100% from scratch, entirely TABOS's own build. Zero lines of Linux code, zero Linux *design* inherited.** TABOS is not a Linux system; nor is it "Linux compatible" like Asterinas/Redox — deliberately the opposite: agents are native citizens, no Linux/POSIX compatibility is targeted.
...
| **MMU page-table FORMAT** (x86 PML4E bits; aarch64 VMSAv8 descriptor) | 🔴 **silicon-mandatory** | Intel SDM §4.5 / ARM ARM D8 | Emits the format (the MMU walks it in hardware); but frame allocator + mapping policy are 100% TABOS |
| **virtio** ring + wire format | 🟢 **open standard (neutral)** | **OASIS** VIRTIO TC (v1.1/1.2 ratified; v1.3 draft) | Writes virtio-mmio drivers *in its own code*; the in-kernel device model is TABOS-native, virtio is a replaceable driver layer |
| **devicetree (DTB)** / PVH start_info | 🟢 **open standard (neutral)** | devicetree.org / Xen | Writes its own parser; the format is OS-neutral |
| **boot handoff** (which register holds what) | 🔵 **TABOS owns** | VMM's choice (not silicon!) | **tb-boot v0** is our own contract; PVH only at bootstrap |
...
| **ELF** boot image container | 🟢 **open standard (neutral)** | System V gABI / TIS | Merely the shell the VMM loader parses; the agent format `.taf` is already TABOS-native |
...
| capability/object-cap security, agent-as-principal, memory-first, DAG-inference | 🟣 **TABOS-novel / neutral-model** | KeyKOS/EROS/seL4 lineage (OS-neutral) | TABOS's core identity |
...
- virtio = **OASIS** TC standard, does not belong to the OS (2-0; correction: v1.3 is still Committee Draft, the ratified ones are 1.1/1.2 — TABOS writes to the ratified version).
...
**Canonical = `tb-boot v0`:** TABOS's owned, frozen, capability-oriented handoff contract. Our own thin VMM (`tb-vmm`) produces it, `tb-hal` consumes it. Enters directly in 64-bit long mode (we write the VMM's initial register file) → **no trampoline, no Linux, no Xen**. boot_params/PVH/Image header are reduced to **compat shims**.
...
- **The in-kernel device model is TABOS-native;** virtio is only a **driver layer**, not a structural bond → replaceable. Long-term `tb-vmm` may offer its own Solo5-style minimal interface.
...
The sourced counterpart of your statement "we are not obliged to carry old bugs". Design decisions of Linux/Unix that are today widely considered **mistakes**, and TABOS's structural alternative:

| Linux/Unix heritage (rejected) | Source/critique | TABOS structural alternative |
|---|---|---|
...
| Kernel | What it did | Relative to TABOS |
|---|---|---|
...
| **Hubris** (Oxide) | No fork/exec, all tasks static | We took spawn-from-manifest from it (but TABOS is dynamic+capability-gated — static is not enough for a self-evolving OS) |
...
| **Redox** | **Counter-example**: deliberately Unix-like | The opposite of TABOS |

**Conclusion:** The decision criterion is "what application-compatibility do you want". TABOS's citizen is the **native agent**; legacy application compat is not required → TABOS owes **nothing** to the Unix/Linux shape. Native execution principal = **LLM agent** (not Unix process / seL4 thread / Hubris task / Theseus cell) — this is TABOS-novel.
