...
- **The extraction line (separate milestones):** the repo split (portable organ core → `cogitave/agent`); the frozen versioned Yuva↔agent ABI (the capability/syscall surface an EXTERNAL agent artifact builds against); pluggable agent backends; runtime organ ADMISSION of an external agent via the M18.1 gate (the named seam, not built here); M32/host-daemon packaging. `token=extraction=DEFERRED`.
...
- **(B) Compile-out** (stage B, §11) — after the mem/ factorization; a separate reviewed landing with the symbol-absence guard and the wording flip.
