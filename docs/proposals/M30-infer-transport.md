...
title: "M30 — Verified Inference Transport"
description: "Kani-verified inferwire codec plus virtio-console seam for a host-keyed echo channel to a host peer; transport-only, backend=ECHO-ONLY."
tags: ["m30", "transport", "virtio-mmio", "inference", "kani", "tb-vmm"]
timestamp: 2026-06-11T14:58:35+03:00
status: locked
...
**Status:** **landed (stages A+B+C)** — the `inferwire` leaf (+6 measured Kani harnesses, mutation-tested per §6), the two-queue virtio-console kernel seam, the `xport-harness` host echo peer, and the full §5 guard blocks are CI-required on both QEMU-TCG lanes; **stage C (the §3d/§11C tb-vmm backend, `transport=TB-VMM-HOST`) has now landed as its pre-authorized follow-up**: tb-vmm's FIRST `mmio_bus` device (`tb-vmm/src/virtio_mmio.rs` — the modern Version=2 DeviceID-3 register file at scan-slot 0 — + `tb-vmm/src/infer_host.rs` — the in-process host peer reusing this SAME `tb-encode` leaf, including the M31 mock serve loop) with the §5 guard block ported to `run-vmm-x86_64.sh` (cross-process equality against the `--xport-out` witness stream the guest cannot write, key-leak negative, lane cross-pins), closing BACKLOG row 23. Stage-C implementation notes vs this spec: (C1) the §8/§11C `run-vmm` marker bump is written here as M19 → `M30: infer-transport OK`, but M31 stages A+B landed BEFORE stage C — with a peer attached the kernel runs the M31 wire legs too, so the landed lane greps the full CURRENT cumulative tail (`M31: infer-e2e OK backend=MOCK-DETERMINISTIC`; pinning M30 would have left the vmm-lane M31 evidence ungated). (C2) §3d's `--infer-nonce <hex>` + `--xport-key <hex>` CLI shape is superseded by the landed stage-B custody pattern: K+N are BORN from `/dev/urandom` inside the tb-vmm process and never accepted from (or echoed to) any command line — strictly stronger custody; the new flags are `--xport-out` (the leg-2 witness file, a separate capture stream because the peer is in-process) and `--xport-key-out` (the §5.7 leak-check input, the harness `--key-out` twin). (C3) `MAX_EXITS` was bumped 5M → 20M (not the §3d "~20M" guess applied blindly — the measured profile is serial-PIO-dominated and the three virtio sessions add only ~40 MMIO exits each). Original stage A+B implementation notes: (1) §3b's "compiled-in shared key" sentence is superseded by §4's custody rule, which the build follows — K is HOST-custodied per run and revealed ON THE CHANNEL as a 32-byte cleartext trailer after the `ECHO_RESP` frame (`inferwire::INFER_KEY_REVEAL_LEN`), exactly the §12 "key revealed on the channel" reading; the guest image/cmdline never carry it. (2) The per-boot challenge derives from `uhash(label ‖ cycle-counter)` (the §4 fallback; M19's rng selftest returns no bytes to reuse). (3) The §6.5 accumulator harness was RESHAPED down the full, measured #49 mitigation ladder to its named last rung ("split the FrameAccum harness out"): every symbolic/trace form of the garbage-then-full-frame sketch (fully-symbolic bytes → symbolic length → symbolic values only → `kani::solver(kissat)` → a chunked-unwind restructure → a decode-free scan hot path) measured >>120s — a STRUCTURAL CBMC floor for a 66-byte-header protocol (~68 sequential `push_byte` inlines × the per-harness unwind bound on the scan/consume/resync loops, independent of data concreteness). The landed harness proves what model-checking uniquely adds — the at-capacity index/overflow discipline (every index Kani-checked at a tiny const-generic cap, the §6 cap-mutant killer) plus every byte-wise resync class — and the FULL emit trace is a NAMED delegation: 5 dedicated host tests + the Miri gate over the same code, and both live CI boot lanes push the real host response through `FrameAccum` byte-at-a-time every boot with the proven fail-closed `decode` as the arbiter of the emitted window (the kernel stage-0x4 reject). Relatedly, `FrameAccum::scan` does NOT re-inline `decode` per byte: the byte-wise checks enforce exactly `decode`'s header rule set, and every consumer (kernel selftest, host harness, tests) runs the proven `decode` on the emitted window. (4) The §6.4 echo-soundness harness's symbolic flip range covers all tag+body bytes but EXCLUDES the key bytes (129s measured with them, over the 120s budget): key-BIT sensitivity is `kani_khash_tamper`'s theorem at the primitive level, `verify_echo`'s key path is a direct `khash(key, ..)` call, and the wrongkey reject fires in-boot on every attached lane (`wrongkey-rejected=1`) plus in the host tests. · **Pillar:** communication (the sovereignty channel to a host model peer) + sovereignty (tb-vmm grows its first virtio device — stage C) · **Depends on:** M19 (virtio-rng over virtio-mmio), M20 (virtio-blk over virtio-mmio + durable head), M28 (`opframe_rx` MAC seam), **M29 (`tb-encode::khash` — the echo's MAC primitive; HARD sequencing dependency)** · **Tasks:** #87 / A1 infer-transport (sovereignty-plan §M30); promotes the M22 runner-up (`docs/proposals/M22-memory-provenance.md:23`); BACKLOG row 23 (the deferred tb-vmm backend — stage C) · **Marker:** `M30: infer-transport OK`
...
**SPIKE RESULT (2026-06-11, the device-config half — RESOLVED, option A stands).** The exact deployment configuration was booted with the CURRENT M0..M28 kernel on the faithful `run-aarch64.sh` invocation (`-M virt,virtualization=on,gic-version=2,iommu=smmuv3 -global virtio-mmio.force-legacy=false`) plus the three M30 additions (`-chardev socket,server=on,wait=off` + `-device virtio-serial-device` + `-device virtconsole`), on BOTH pinned QEMU builds:
- **qemu-6.2.0** (local WSL Ubuntu-22.04): full chain PASS to the `M28: operator-cmd OK` tail, qemu rc=0, chardev unix socket created.
- **qemu-8.2.2** (the CI build, `tabos-qemu8` container): identical result — full chain PASS, rc=0, socket created.
Established: the device instantiates on the virtio-mmio bus alongside rng+blk on both builds; **the existing M19/M20 DeviceID-matching slot scan tolerates the third device** (no slot-shift regression — the exact deployment config of stage B is already green against today's kernel); the host-side socket endpoint exists for the harness peer. STILL OPEN (needs the M30 guest driver, stage B): the guest-side Version=2 register readback on the new slot and the non-F_MULTIPORT port-0 negotiation — the spike de-risks the config; the driver proves the negotiation.
