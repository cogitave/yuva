...
**Status:** proposed (research-complete, build not started) · **Pillar:** communication (the M28 MAC honesty upgrade) + memory (the #74 provenance-hash first half) · **Depends on:** M22 (provenance fold), M28 (`opframe_rx` + the `mac=KEYED-NONCRYPTO` concession) · **Tasks:** the M28 §5 named successor; #74 (hash half); #75 (enabler) · **Marker:** `M29: khash-mac OK`
