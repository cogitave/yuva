...
> Status: the agent-native milestone chain **M0 → M30 is complete and CI-green on
...
> the Kani-proven `tb-encode::inferwire` frame codec (house magic `0x5444`,
...
## 2. The milestone chain (M0 → M30, + L2.0 … L2.6)
...
witness line carries the machine-emitted prove/assume boundary), and finally the
NEW cumulative-tail `M30: infer-transport OK` verified-inference-transport marker
...
line — cross-process equality with a host-custodied key)
(each preceded by its anti-hollow witness line: `prov: …`, `exp: …`, `bakeoff: …`,
`opframe: …`, `exittel: …`, `khash: …`, `opcmd: …`, `xport: …`) — the ordered
sequence (detailed through M22) is listed further below.
...
| TABOS tb-boot | `TABOS` / `0x54420001` (`TB_NOTE_TYPE_ENTRY64`) | `_tb_start` (64-bit) | `tb-vmm` *(historical TABOS-era wire string, migrating in the brand PR)* |
...
marker; the tail has since moved along the chain and is `M30: infer-transport OK`
today). The six
...
regression of M0 through the cumulative tail `M30: infer-transport OK` (the listing
below is detailed through M22; after it the M23→M26 learning-loop arc, the M28
operator-inbound marker, the `M29: khash-mac OK` keyed-MAC marker and the NEW `M30: infer-transport OK` tail print in the §2 order
