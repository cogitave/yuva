...
> admission-only + inert in the all-mock chain), all organs are mocks at M38 landing (the real M32
> LOCAL organ has since landed at M32 stage B — #90 CLOSED — as the Conductor's
> deterministic local stand-in, `m32b=DETERMINISTIC-STAND-IN`); the NEW cumulative-tail marker `M38: conductor OK
...
| **M33 (stage A+B)** | **Provenance lineage — a Kani-verified LMS *verify* leaf + a DSSE/SLSA attestation codec + a host LMS signer + the PERSISTED signed head (#91 — CLOSED at stage B via the two-boot cross-boot witness).** PARTICIPATION→EXCLUSIVITY: M29 made the prov head a *keyed MAC* (symmetric — anyone holding K can mint a head); M33 adds a **hash-based signature** (LMS, RFC 8554, LM-OTS `W4` over tree `H10`) whose public root is a 32-byte image constant and whose private key NEVER enters the kernel — so only the signing-key holder can extend the signed head and ANY party verifies with only the root. The **VERIFY/SIGN split** (proposal §2): the kernel + Kani lane see ONLY `tb-encode::lmsig::lms_verify` (`#![no_std]`, zero-dep, forbid-unsafe, SHA-256 via the NEW `tb-encode::sha256` leaf — RFC 8554 pins SHA-256, so a SECOND in-house verified hash leaf is paid for to keep `conformance=RFC8554`); signing + the never-reuse OTS leaf-index live host-side (`tools/prov-signer`, cfg-gated OUT of the kernel TCB, a nested workspace). **Ed25519/curve crypto REJECTED** (the `2^255-19` field arithmetic is the documented CBMC SAT-explosion case — Kani-INFEASIBLE); **SPHINCS+ REJECTED** as the kernel-verify primitive (≈100× khash cost). The DSSE-PAE-shaped attestation codec (`tb-encode::attest`, a fixed-width in-toto-Statement subset — NOT a JSON producer) doubles as the machine-readable sovereignty-ledger carrier, reaching SLSA **L1→toward L2**. Kani (the NEW **shard C**, the 3-way split + the 30→40-min headroom bump): 8 harnesses — the SHA-256 leaf (`kani_sha256_total`/`kani_sha256_kat`), the LMS verify leaf's **`w=1` TOY instance** (`kani_lms_verify_total`/`_tamper` [the pinned-vector iff, symbolic ROOT — the M31 idiom, keeping every SHA-256 compression concrete]/`_otschain_step`/`_merklepath`; a full-parameter verify is ~1062 SHA-256 compressions, CBMC-INFEASIBLE — so the toy is proven + the OFFICIAL RFC 8554 Appendix F Test Case 1 vector is the host `cargo test` KAT + the small-parameter in-boot KAT), and the DSSE-PAE codec (`kani_attest_pae_injective`/`kani_attest_decode_fail_closed`). HONEST SCOPE (machine-emitted, load-bearing): a signature proves `exclusivity=OFF-PLATFORM-ONLY` (against parties OUTSIDE the signing host) and **NOTHING** against the host holding the key; `key=SIMULATED-ENROLLED-CI-CUSTODIED`; `state=SIMULATED-REUSE-OK-NO-SECURITY` (the stage-A CI lane DELIBERATELY reuses leaf index 0 — acceptable ONLY because the simulated key has no security value; a durable never-decrement counter is the M35 (see `docs/plans/sovereignty-plan.md`) obligation); `measure=SELF-NO-HW-ROOT selfmeasure=UNATTESTED-LOADER` (no measured boot/RTM; the subject digest is self-reported by the attested image); `splitview=UNDETECTED-NO-WITNESS-QUORUM`; `sec=ASSUMED-FROM-LITERATURE` (LMS EUF-CMA + SHA-256 resistance assumed, never proven). **Stage B (LANDED — closes #91):** the compiled-in full-parameter `W4`/`H10` signed head (`lmsig::PROV_KAT_*`, a signer artifact regenerated by a host test — the private key never enters the kernel) is PERSISTED to a ping-pong pair of on-disk slots ABOVE the M20 partition via a NEW multi-sector torn-write-safe codec (`tb-encode::provhead`: a record-spanning FNV-64 catches a torn MIDDLE sector, a per-sector `gen_tag` catches a mixed-gen torn commit, a two-phase-`gen` ping-pong recovers the prior consistent head), and on a later boot READ BACK + `lms_verify`-checked — so a SIGNED head survives a genuine reboot. The two-boot cross-boot witness (boot 1 persists on a fresh disk → `head-persisted=0x1 head-reboot-survived=0x0`; boot 2 against the SAME M33 sectors → both `0x1`, the `head=` read FROM DISK string-equal to boot 1's, on BOTH arches) upgrades the marker `M33: prov-lineage verify OK` → **`M33: prov-lineage OK`**. Every-boot the kernel also verifies the compiled-in full `W4`/`H10` signature (a full-parameter verify KAT, stronger than the toy KAT). | `M33: prov-lineage OK` (guest serial, both arches) + the two-boot `prov-sig: … head-persisted=0x1 head-reboot-survived=0x1` witness + shard-C Kani `V1-shard-c: kani-encoders OK` | +8 stage-A `kani_sha256_*`/`kani_lms_*`/`kani_attest_*` + 2 stage-B `kani_persisted_record_decode`/`_recover` harnesses → **122** at M33's landing (shard C = the 6 crypto + 2 codec harnesses; the 2 cheap attest layout harnesses joined shard A) — **M32-B (#68) has since added the +1 infer-local decode harness `kani_inferwire_infer_peer_bound`, so the in-code `EXPECTED_HARNESSES_TOTAL` is now 123**; the kernel stays zero-unsafe and verifies against the image-embedded 32-byte public root |
| **M32 (stage A + stage B)** | **The local inference daemon — the FIRST machine-tokened sovereignty debt (#90 — CLOSED at stage B).** A host-adjudicated, CPU-only, **zero-network-at-runtime, zero-secret** lane (`m32-local-infer.yml`): a memory-safe Rust **daemon** (`tools/infer-daemon/`, a nested workspace) holds the channel key + the artifact SHA256 pins + ALL witness emission and supervises a **keyless, env-cleared, seccomp+Landlock-self-sandboxed engine WORKER** that links vendored llama.cpp (pinned `b9756` @ `d0f9d2e5`, source-archive SHA256-pinned) behind a narrow flat-C `-sys` crate (`llama-engine-sys`) and parses the SHA256-pinned GGUF ONLY after the sandbox filter is installed. The debt is **machine-tokened on every witness line** (`engine=VENDORED-C-LLAMACPP debt=SOVEREIGNTY-OPEN-B3 memsafety=UNSAFE-C-PROCESS-CONFINED weights=UNTRUSTED-INPUT-NAMED`) and grep-locked **bidirectionally** (the DEBT-LOCKSTEP job: dep-present∧token-absent FAILS, dep-absent∧token-present FAILS, `engine=PURE-RUST` while the C remains FAILS, the daemon lockfile carrying any network dep FAILS, and the **`CC=/bin/false` poison probe** against a pristine target dir — the worker build MUST fail, proving the C is genuinely in the build graph). Determinism is EARNED per run inside the §4 envelope (single slot, no prompt cache, pinned source build, `n_threads=1`, greedy `top_k=1`, **baseline ISA — no AVX dispatch at all** for cross-runner bit-exactness): a fresh-process 2-run compare + a frozen NEG-PROMPT (own golden, measured-distinct) + a per-run nonce-freshness 2-run, each adjudicated against **repo-pinned goldens at the workflow level** (`scripts/m32-adjudicate.sh` — the independent stream: repo constant vs daemon stdout; the witness nonce must equal the workflow-sampled nonce — the canned/replay killer). Two CI models: `stories260K.gguf` (committed in-repo, the network-independent smoke) + `stories15M-q8_0.gguf` (the quantized-kernel path, hash-keyed cache + setup-step re-fetch on miss). HONEST SCOPE: `backend=LOCAL-ENGINE` proves an owned PROCESS ran an inference, NOT sovereignty (the code is vendored C, the debt is open); `memsafety=UNSAFE-C-PROCESS-CONFINED` is the CEILING (OS process + seccomp/Landlock hardening, NOT an isolation boundary); `weights=UNTRUSTED-INPUT-NAMED` survives the hash pin; `guestpath=MOCK-PINNED-HOST-ADJUDICATED` — no local-engine byte reaches the guest at stage A; `digest=TUPLE-CONDITIONAL`. **Two named deviations:** the ISA is `X86-64-BASELINE-PINNED` (the proposal's §4 honesty fallback over `X86-64-V3-PINNED`); the stage-A witness is DECOUPLED (the daemon is not the boot's chardev peer, leaving the cumulative chain + run scripts byte-identical). The marker `M32: local-infer OK backend=LOCAL-ENGINE` lives ONLY in the workflow summary — it NEVER enters the cumulative chain. **Stage B (#68, closes #90) LANDED the kernel RECEIVE path** — the M38 Conductor's real LOCAL organ: a deterministic stand-in exchanged BESIDE the untouched M31 mock leg (`peer_id` asserted `== 0x03` so a `0x02` mock frame can never wear the local identity, MAC-verified per chunk under `YUVA-M31-INFER-V1`, the per-run host nonce carried through, digest-committed, bit-exact vs the in-kernel stand-in) that FEEDS the Conductor's round-1 refinement, witnessed by the guest `infer-local:` line + the §8.6 CROSS-PROCESS leg (its resp-digest string-equals the host `xport-local:` digest, separate capture streams) + the `kani_inferwire_infer_peer_bound` proof (`infer_tag` peer-pair `0x02`/`0x03`/`0x04` distinctness, `EXPECTED_HARNESSES_TOTAL 122→123`, shard C); `m32b=DETERMINISTIC-STAND-IN` (honest — a stand-in, NOT a model). | `M32: local-infer OK backend=LOCAL-ENGINE` (summary-only) + the guest `infer-local:` witness (stage B) | host-side std Rust + vendored C OUTSIDE the Kani surface (its evidence is the §4 runtime four-leg compare + the §8 workflow guards, honestly NOT proofs); stage B adds ONE `tb-encode` harness (`kani_inferwire_infer_peer_bound`) + the kernel receive seam, no new wire kinds / MAC domains |
| **M40** | **Verified LEXICAL recall-scoring leaf (#76, Track A of `docs/proposals/forward-plan.md` §3)** — sharpens Cogi's memory retrieval with a BM25-family leaf that is explicitly **NOT semantic**: no embeddings, no vectors, no learned weights, no float (the M21 KAN fixed-point discipline). The three house-pattern pieces: (1) the leaf `tb-encode::recall` (`#![no_std]`, forbid-unsafe, zero-dep) — `bm25_idf` (the former M13-inline IDF hoisted verbatim, reusing `memscore::ln_fixed`), `bm25_tf_norm` (tf saturation + doc-length normalization), `bm25_term_score`/`bm25_doc_score` (multi-term additive, saturating), and the fixed-width injective `hit_canon`/`hit_decode` ranked-hit codec; (2) the seam — `mem/mod.rs` recall computes its BM25+IDF via `recall::bm25_idf` (numerically identical, ranking byte-identical) and `mem/selftests.rs::recall_selftest` scores a deterministic KAT AND drives the real organ recall; (3) the marker — a profile-gated `M40: recall OK` plus an honest `recall:` witness (substrate takes the skip form, `recall:` census-forbidden). SP#4 — the M38 conduct head is verified **byte-identical** (`0x066855300c57557b`): the seam is value-preserving and the conductor consumes recall only as a gate (`ctx_recalls == 3`) over a hardcoded `context_strength`, discarding the returned record id. HONEST SCOPE (machine-emitted): `retrieval=LEXICAL-BM25-NO-FLOAT recall=DETERMINISTIC idf-source=VERIFIED-LEAF k1=0x4b0 b=0x2ee embedding=NONE weights=NONE-FROZEN-FIXED-POINT sec=ASSUMED-FROM-LITERATURE`. | `M40: recall OK` + the `recall: top-id=… ranking-ok=0x1 monotone-ok=0x1 bounded-ok=0x1 canon-ok=0x1 organ-ok=0x1 semantic=0x0 …` witness | +7 `kani_recall_*` harnesses (panic-freedom + bounds + codec injectivity/fail-closed + accumulation-monotonicity), 128→**135**; the shard split rebalances FOUR-WAY (A/B/C/D, `scripts/kani-shards.sh`) |
...
- **The forward plan + the stage-A separability wave (LANDED).** The
  ...
  **Yuva-ABI stage A** (#65 — the normative [`spec/yuva-abi-v1.md`](spec/yuva-abi-v1.md) +
  ...
  PR in flight), with Cogi's own model a Phase-2 BRAIN-gated step. tinygrad and
  Namzu are DEFERRED; the HARDWARE / BRAIN / EXTRACTION gates stay operator-owned.
