...
> Since then the chain has grown six follow-on markers — **M14.1** (byte-payload IPC), **M14.2** (blocking-recv), **M15.1** (block unmap + frame reclamation), **M18.1** (mandatory human-approval gate), **M18.2** (rotating held-out evaluator), and **M19** (a poll-based virtio-mmio virtio-rng round-trip — the kernel's FIRST real device I/O, now the LAST cumulative marker) — plus a **formally-verified core**: the M11 rights-subset invariant is Kani-proven (`crates/tb-caps-core`, `M11: caps-subset PROVEN`), the silicon-unsafe encoders/parsers are Kani-proven (`crates/tb-encode`, `V1: kani-encoders OK`), and a Miri Tier-0 UB gate runs over both (`T0: miri OK`). CI now has **six lanes** (ci, vmm-boot, l2-nested-vmx, microvm-kvm, kani, miri). See §6.
...
| **M19** (poll-based virtio-mmio device I/O — the FIRST real device I/O) | ✅ **complete**, CI-green (x86_64 microvm + aarch64 virt; the LAST cumulative marker, printed AFTER the L2.0 lines) — a poll-based **virtio-mmio** virtio-rng round-trip in NEW `crates/tb-hal/src/arch/{x86_64,aarch64}/virtio.rs`: arch-neutral slot scan (MagicValue/DeviceID; x86 base `0xFEB00000` UC-mapped at `LAPIC_WINDOW_VA+0x1000`, aarch64 base `0x0A000000` already Device-mapped), modern (Version=2) handshake, one queue with desc/avail/used + buffer in ONE identity-mapped `frame_alloc` frame, submit→`QueueNotify`→**poll `used.idx`** under a fail-closed `POLL_CAP=100M` (never hangs), `NO_INTERRUPT` set. Safe `virtio_selftest()→VirtioProof{Absent\|LegacyUnsupported\|Proven\|Failed}` facade; kernel zero-unsafe. **Proven under TCG (`ci`) + KVM (`microvm-kvm`)** with `-device virtio-rng-device -global virtio-mmio.force-legacy=false`; **graceful Absent-skip under tb-vmm** (open-bus `0xFFFFFFFF`≠magic → `M19: virtio OK (no device, skipped)`, so `vmm-boot` stays green with no tb-vmm backend). DEFERRED: the completion-IRQ path (aarch64 GIC SPI 48 first, then an x86 IOAPIC driver), the M14-stream integration, and the tb-vmm host-side virtio backend |
| **Verification — Kani (M11 caps + encoders) + Miri Tier-0** | ✅ **landed**, three machine-checked gates: the M11 rights-subset / no-confused-deputy invariant is Kani-proven over `crates/tb-caps-core` (the single source of truth the kernel runs verbatim — zero model drift; **12 harnesses**, `M11: caps-subset PROVEN`, `kani.yml` `prove-caps` job); the silicon-unsafe encoders/parsers (control-MSR adjust-legality gate, CR0/CR4 fixed-bit clamp, page-table/EPT encoders, the aarch64 stage-2 descriptor + `VTCR_EL2`/`VTTBR_EL2` algebra, the `ESR_EL2`/`HPFAR_EL2` trap-syndrome decoders, the 16-byte IPC frame round-trip + fail-closed decode, a bounded no-alloc ring) are Kani-proven over a NEW `crates/tb-encode` (**20 harnesses** — `EXPECTED_HARNESSES`, the 15 prior + the 5 new L2.1 stage-2/`el2_trap` lemmas — `V1: kani-encoders OK`, `kani.yml` `prove-encode` job); and a Miri Tier-0 UB gate runs `cargo miri test -p tb-caps-core -p tb-encode` over the EXACT host-runnable leaf code the kernel runs (`T0: miri OK`, `miri.yml`). `tb-hal` + `kernel` are excluded from Miri (asm + `os=none` triple) |
| **CI lanes (6)** | ✅ green/allow-skip — `ci` (both-arch QEMU-TCG cumulative-marker boot, M0..M19), `vmm-boot` (`tb-vmm`/KVM via the sovereign `tb-boot v0` contract; allow-skip when KVM absent), `l2-nested-vmx` (the REAL L2.0 VMX-root proof under nested KVM; allow-skip when nested VMX absent), `microvm-kvm` (the THIRD boot lane — QEMU-`microvm`+KVM boot-assert + the `--release` boot-ready-cycles bench feeding BENCHMARKS §3), `kani` (2 jobs: `prove-caps` + `prove-encode`), `miri` (the Tier-0 UB gate) |
...
- **The isolation property is proven, not merely booted.** Following the pKVM/SeKVM precedent that a guest's stage-2 only ever maps frames it owns or was explicitly shared ([pKVM Security Model](https://source.android.com/docs/core/virtualization/security)), the new pure stage-2 descriptor algebra (`tb-encode/stage2.rs`) and `ESR_EL2`/`HPFAR_EL2` trap-syndrome decoders (`tb-encode/el2_trap.rs`) are Kani-proven host-side and reduce to the SAME M11 rights-subset / no-confused-deputy attenuation invariant this chain already machine-proves over `tb-caps-core` — stage-2 confidentiality is an instance of an existing proof, not a new mechanism. The `prove-encode` lane's pinned `EXPECTED_HARNESSES` grew 15 → 20 (the 5 new L2.1 stage-2/`el2_trap` lemmas), bumped in lockstep with `kani.yml`. See [MILESTONES](MILESTONES.md) for the cumulative marker chain.
