...
| **M14** (inter-agent IPC — capability-passing channels + ordered streams) | ⏳ **in progress** (next increment) — fills the body behind the born-with `ObjKind::Channel` bootstrap handle; lights up the M13 `copy_to_user`/`copy_from_user` buffer payload path the pointer-free M13 ABI was designed to forward to |
| M15 – M18 | ⬜ planned (this document) |
