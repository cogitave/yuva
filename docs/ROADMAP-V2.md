...
- **M31 stage C — the ANTHROPIC-LIVE bridge (the OPERATOR'S lane): ✅ LANDED (the infrastructure; the hello itself is the operator's dispatch).** `ureq`+`rustls`+`serde_json` in the host harness only (`tools/xport-harness/src/live.rs` — the LANGUAGE-AND-STANDARDS §0/§6 [DECISION] rows were pre-landed; the model `claude-haiku-4-5` and `max_tokens=64` are compile-time consts, the operator's spend pin), `real-infer.yml` on `workflow_dispatch` ONLY (never `pull_request_target`, never a required check, missing secret = the LOUD named skip), the §5 liveness protocol on the kernel's per-boot wire challenge — transform v3 `HEX-REVERSE-ANY` (accepts ANY of the three standard reversals of the challenge hex — group-order, byte-order, or char-order — naming the matched form in `matched=GROUP|BYTE|CHAR`; v2 accepted ONLY the group-order reversal and FAILED a real 200, run 27959048211, whose model produced the byte-order reversal `a1b16c92...` for challenge `8c3f1a58...`; v1's character-level reversal was a tokenization-unrealistic CAPABILITY test, run 27408247558 — each generation fail-closed as designed) — host leg: HTTP 200 + request-id + any standard reversal verified; the HTTP layer is a fixture-testable seam, the key-scrub is unit-tested, ONE call per run is latched; what the model said is hex-framed on the body line on OK AND on the 200-class failures (TRANSFORM-MISS/REFUSAL), decoded only on the run-summary page. The workflow adjudicates the bridge witness + the UNTOUCHED cumulative chain and prints the dual-marker verdict `M31: real-infer OK backend=ANTHROPIC-LIVE` at the WORKFLOW level. **Honest stage-C scope:** the guest exchange stays the bit-exact deterministic mock shape on every lane (the stage-B kernel pins it — `selftests.rs` 0x21/0x25/0x26), so the live response is host-witnessed by DIGEST and never rides to the guest; the kernel-leg liveness (§5.5: the in-guest transform search, the guest-serial live witness/marker, the live digest in the M25 fold) is the NAMED kernel follow-up. Operator runbook: [`docs/RUN-THE-HELLO.md`](RUN-THE-HELLO.md). Named successors: the §5.5 kernel leg, **M32** (the local llama.cpp daemon serving `model:local/llama` over the SAME INFER_REQ/INFER_RESP framing, #90), **M33** (a signature primitive — host participation → host exclusivity; signed prov heads carry the inference digests), **B2** (the vsock-only `model:` API — the inferwire frame migrates onto a vsock stream unchanged), plus the named M31 deferrals: byte-payload M13 memory records and static registration of a channel-backed route.
- **M32 — the local inference daemon (#90): research-first proposal LANDED (docs-only — nothing implemented).** [`docs/proposals/M32-local-infer.md`](proposals/M32-local-infer.md) + the co-landed survey [`docs/research/m32-local-infer-literature.md`](research/m32-local-infer-literature.md), adversarially reviewed 2× SOUND-WITH-AMENDMENTS (all applied): vendored llama.cpp behind a keyless, seccomp+Landlock-self-sandboxed engine worker on the SAME M30/M31 channel, carried as the FIRST machine-tokened sovereignty debt (`engine=VENDORED-C-LLAMACPP debt=SOVEREIGNTY-OPEN-B3`, the bidirectional DEBT-LOCKSTEP grep guard, the B3 retirement acceptance pre-registered with frozen constants); staged A (host-adjudicated daemon lane, both arches, zero-network-at-runtime/zero-secret) then B (the kernel receive path — #90 closes at stage B, never at A); the cumulative tail stays `M31: infer-e2e OK backend=MOCK-DETERMINISTIC` permanently.
- **M33 — provenance lineage (#91): ✅ LANDED (stage A + stage B — #91 CLOSED).** PARTICIPATION→EXCLUSIVITY. Stage A: the Kani-verified LMS *verify* leaf (`tb-encode::lmsig`, RFC 8554 `W4`/`H10`) + the second in-house SHA-256 leaf (`tb-encode::sha256`, `conformance=RFC8554`) + the DSSE-PAE attestation codec (`tb-encode::attest`) + the host signer (`tools/prov-signer`, private key NEVER in the kernel) + the in-boot toy KAT with two regional tamper controls. **Stage B (closes #91):** the compiled-in full-parameter `W4`/`H10` signed head is PERSISTED to on-disk ping-pong slots via a NEW multi-sector torn-write-safe codec (`tb-encode::provhead` — a record-spanning FNV-64 catches a torn middle sector, a per-sector `gen_tag` catches a mixed-gen torn commit, and a two-phase-`gen` ping-pong recovers the prior consistent head), and on a later boot READ BACK + `lms_verify`-checked — a SIGNED head survives a genuine reboot. The two-boot cross-boot witness (boot 1 fresh-disk persist → `head-reboot-survived=0x0`; boot 2 against the SAME M33 sectors → `0x1`, the `head=` read FROM DISK string-equal to boot 1's, on BOTH arches) upgrades the marker to **`M33: prov-lineage OK`**. Kani: shard C = 6 crypto + 2 stage-B codec harnesses (`kani_persisted_record_decode`/`_recover`), total **122**. HONEST (machine-tokened): `exclusivity=OFF-PLATFORM-ONLY key=SIMULATED-ENROLLED-CI-CUSTODIED state=SIMULATED-REUSE-OK-NO-SECURITY measure=SELF-NO-HW-ROOT selfmeasure=UNATTESTED-LOADER sec=ASSUMED-FROM-LITERATURE`. Named successors: #75 (batch-Merkle, composes by construction), #85 (real enrolment, custody rung 2), M35 (the durable never-decrement leaf-index counter + cross-run persistent-disk lane), M36 (SLSA L3).
