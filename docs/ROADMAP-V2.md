...
| **M13** (default tiered memory substrate) | ⏳ **in progress** (next increment) — fills the body behind the born-with `ObjKind::MemoryHome` handle (T0/T1/T2 tiers + lexical T3 + `tb_mem_*` ABI), no handle/manifest/ABI change |
| M14 – M18 | ⬜ planned (this document) |
