...
- **The isolation property is proven, not merely booted.** Following the pKVM/SeKVM precedent that a guest's stage-2 only ever maps frames it owns or was explicitly shared ([pKVM Security Model](https://source.android.com/docs/core/virtualization/security)), the new pure stage-2 descriptor algebra (`tb-encode/stage2.rs`) and `ESR_EL2`/`HPFAR_EL2` trap-syndrome decoders (`tb-encode/el2_trap.rs`) are Kani-proven host-side and reduce to the SAME M11 rights-subset / no-confused-deputy attenuation invariant this chain already machine-proves over `tb-caps-core` — stage-2 confidentiality is an instance of an existing proof, not a new mechanism. The `prove-encode` lane's pinned `EXPECTED_HARNESSES` grew to 46 (the 5 new L2.1 stage-2/`el2_trap` lemmas were the first increment, since extended across L2.2–L2.6 and the M20/M21/M22 leaves), bumped in lockstep with `kani.yml`. See [MILESTONES](MILESTONES.md) for the cumulative marker chain.
