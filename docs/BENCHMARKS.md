...
QEMU/Firecracker, the brand (YUVA) note for `tb-vmm`) mean **no SeaBIOS/OVMF, no
...
- **Quotable figures come from a `--release` build under `tb-vmm`, timed by an
  ...
  rule, 2026-06-07.) A `debug/yuva-kernel` wall-clock number under QEMU/KVM
