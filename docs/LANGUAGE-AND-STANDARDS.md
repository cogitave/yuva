...
| **Supply chain** | **SLSA v1.0** Build L1→L3; **in-toto** provenance | Every artifact (kernel image, node image, WASM tool bundles, SDK crates/wheels) ships with provenance from hosted CI. **M33 (stage A+B, LANDED — #91 closed)** lands the DSSE-PAE-shaped attestation codec (`tb-encode::attest`, a fixed-width in-toto-Statement subset — NOT a wire-compatible JSON producer) reaching SLSA **L1→toward L2**, plus (stage B) the persisted signed head via the NEW multi-sector torn-write-safe `tb-encode::provhead` codec + the two-boot cross-boot witness (`M33: prov-lineage OK`); L3 (`repro=BIT-IDENTICAL-2BUILD`) is M36. `measure=SELF-NO-HW-ROOT selfmeasure=UNATTESTED-LOADER` (no measured boot/RTM; the subject digest is self-reported by the attested image) |
| **Signature / prov-lineage leaf** **[DECISION]** | **LMS (RFC 8554)** in-house, SHA-256 leaf (`tb-encode::lmsig` VERIFY-only + `tb-encode::sha256`); the LMS signer is host-side (`tools/prov-signer`, cfg-gated OUT of the kernel TCB) | Sovereignty-ledger status: the LMS verify leaf + the SHA-256 leaf + the stage-B multi-sector persisted-head codec (`tb-encode::provhead`, torn-write-safe: record-spanning FNV-64 + per-sector `gen_tag` + two-phase-`gen` ping-pong) are **ACCEPTED-PERMANENT** (in-house zero-dep, the cost of public-verify/private-sign exclusivity over M29's symmetric MAC); the simulated signing key is **TEMPORARY** (rung-0 `key=SIMULATED-ENROLLED-CI-CUSTODIED`; real operator enrolment is #85). `sec=ASSUMED-FROM-LITERATURE` (LMS EUF-CMA + SHA-256 resistance assumed, never proven — the khash claim tier); the Kani obligation is a `w=1` toy instance + the official-vector host KAT (a full-parameter verify is ~1062 SHA-256 compressions, CBMC-infeasible) (M33 proposal §4/§9) |
